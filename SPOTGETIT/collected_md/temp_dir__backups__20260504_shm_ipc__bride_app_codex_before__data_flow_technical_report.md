...
| Jetson 명령 송신 | `jetson_tx.c` | `jetson_tx_thread()` | Qt Unix socket 명령을 Jetson UDP 9001로 송신 |
...
| Qt -> BridgeDaemon 명령 | Unix Domain Socket | `/tmp/bridge_cmd.sock` | `proto.h` |
...
| 단계 | 파일 | 함수/코드 |
|---|---|---|
| Unix socket 서버 생성 | `jetson_tx.c` | `create_unix_server()` |
| client accept/select | `jetson_tx.c` | `jetson_tx_thread()` |
| `CmdPacket` 수신 | `jetson_tx.c` | `handle_gui_packet()` |
| priority/ACK flag 결정 | `jetson_tx.c` | `handle_gui_packet()` |
...
PC와 Qt의 명령은 입력 통로만 다르고, 최종 Jetson 송신은 같은 `bridge_api_send_command()`를 사용한다.
...
| Qt 명령 | Qt GUI | `jetson_tx_thread` | `send_cmd_to_jetson()` | UDP 9001 | Jetson |
...
| Qt 명령 Unix socket | `jetson_tx.c:41` | `create_unix_server()` |
| Qt 명령 packet 처리 | `jetson_tx.c:62` | `handle_gui_packet()` |
