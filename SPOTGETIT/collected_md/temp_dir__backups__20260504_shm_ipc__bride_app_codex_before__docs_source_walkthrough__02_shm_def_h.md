...
`shm_def.h`는 BridgeDaemon과 Qt GUI가 공유하는 POSIX shared memory layout을 정의한다. daemon은 writer, Qt는 reader다. 두 process가 같은 구조체를 같은 offset으로 해석해야 하므로 이 파일은 ABI 문서에 가깝다.
...
| 32 | `<pthread.h>` | POSIX | process-shared rwlock |
