...
| 일반 mutex | POSIX pthread mutex | `frag_queue.h`, `bridge_api.c`, `jetson_rx.c`, `protocol_timer.c` |
...
| `jetson_tx_thread` | Qt GUI command 수신 및 Jetson 송신 | stop flag, BridgeApi |
...
| 파일 | 사용 기법 | 설명 |
|---|---|---|
| `shm_def.h` | atomic, rwlock, semaphore | SHM ABI에 동기화 객체 포함 |
| `bridge_main.c` | atomic stop, SHM 생성, sem/rwlock init, pthread create/join | daemon lifecycle 관리 |
...
| `jetson_tx.c` | atomic stop | Qt command socket loop 종료 제어 |
