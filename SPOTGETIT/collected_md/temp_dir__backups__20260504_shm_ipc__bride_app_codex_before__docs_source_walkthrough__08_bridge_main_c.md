...
bridge_main.c의 main()은 한마디로 BridgeDaemon 전체 자원 만들기 → thread context 배선 → thread 실행 → watchdog 대기 → 종
  ...
  새로 만드는 파일/SHM/socket 등의 권한이 umask 때문에 깎이지 않게 한다. 이 daemon은 /dev/shm 공유메모리나 Unix socket 같은
  ...
  jetson_tx_thread는 Qt GUI 쪽 Unix socket에서 command를 받아 Jetson UDP로 보낸다.
  ...
  그래서 TX thread는 Jetson 주소 table과 BridgeApi가 필요하다.
